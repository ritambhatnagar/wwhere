...
class linkedin {
    ...
    function linkedin($data) { //echo "come";pr($data);exit;
     ...
     * @return type
     */
    function share($comment, $title, $url, $image_url, $access_token, $description = "One of the aspect of Leanbusiness.io is simplest way to manage your brand across multiple social media. Social, Brand, Marketing, Inbound") {
    ...
    function fetch($consumerArray, $url, $access_token) {
        $shareUrl = 'http://api.linkedin.com' . $url;
        $this->consumer = new OAuthConsumer($consumerArray['consumer_key'], $consumerArray['consumer_secret']);
//pr($consumerArray);       
//pr($access_token);exit;
    ...
    function getProfile($resource = "~") {
        $profile_url = "http://api.linkedin.com/v1/people/" . $resource;
